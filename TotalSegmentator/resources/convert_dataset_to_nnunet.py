...
def generate_json_from_dir_v2(foldername, subjects_train, subjects_val, labels):
    ...
    # json_dir = os.path.join(out_base,"dataset.json")
    # os.makedirs(json_dir, exist_ok=True)
    print(out_base)
    os.makedirs(out_base, exist_ok=True)
    dict_dir = os.path.join(str(out_base), "dataset.json")

    with open(dict_dir, "w") as json_file:
        json.dump(json_dict, json_file, sort_keys=True, indent=4)

    print("Creating split_final.json...")
    # output_folder_pkl = Path(os.environ['nnUNet_preprocessed']) 
    ...
    nnUNet_results = Path(os.environ['nnUNet_results']) / foldername
...
if __name__ == "__main__":
    ...
    print(dataset_path)
    nnunet_path_all = Path(sys.argv[2])  # directory of the new nnunet dataset

    name = nnunet_path_all.name

    # nnunet_path = os.path.join(nnunet_path_all, "nnUNet_raw")
    # os.makedirs(nnunet_path, exist_ok=True)
    # nnunet_path = Path(nnunet_path)

    nnunet_path = Path(os.environ['nnUNet_raw']) / name
    ...
    generate_json_from_dir_v2(name, subjects_train, subjects_val, class_map.values())
